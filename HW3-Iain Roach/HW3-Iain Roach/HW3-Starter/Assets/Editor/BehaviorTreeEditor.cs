...
public class BehaviorTreeEditor : EditorWindow
{
    ...
    void DrawWindow(int windowID)
    {
        ...
        // This formats the Is Here window
        else if (TypeToString(idToNode[windowID].GetType()) == "IsHere")
        {
            ...
            if (GUI.Button(new Rect(5, 45, 190, 20), "Grabber: " + isHereTask.Grabber.ToString()))
            {
                GenericMenu menu = new GenericMenu();
                foreach (Character character in Enum.GetValues(typeof(Character)))
                {
                    ...
                    {
                        Debug.Log("Grabber: " + characterString);
                        isHereTask.Grabber = character;
                        idToNode[windowID] = isHereTask;
                    });
                }
                menu.ShowAsContext();
        ...
        // This section formats the Pickup
        else if (TypeToString(idToNode[windowID].GetType()) == "PickUp")
        {
            PickUp pickUpTask = idToNode[windowID] as PickUp;

            if (GUI.Button(new Rect(5, 20, 190, 20), "Pickuper: " + pickUpTask.Pickuper.ToString()))
            {
                GenericMenu menu = new GenericMenu();
                foreach (Character character in Enum.GetValues(typeof(Character)))
                {
                    ...
                    menu.AddItem(new GUIContent(characterString), false, () =>
                    {
                        Debug.Log("Pickuper set to: " + characterString);
                        pickUpTask.Pickuper = character;
            ...
            if (GUI.Button(new Rect(5, 45, 190, 20), "Pick up this: " + pickUpTask.PickupThis.ToString()))
            {
                GenericMenu menu = new GenericMenu();
                foreach (Thing thing in Enum.GetValues(typeof(Thing)))
                {
                    ...
                    menu.AddItem(new GUIContent(thingString), false, () =>
                    {
                        Debug.Log("Pickup this set to: " + thingString);
